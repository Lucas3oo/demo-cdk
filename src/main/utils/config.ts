...
export function getEnvConfig(app: App): any {
  ...
  if (!envConfigName) { throw new Error('Context variable missing on CDK command. Pass in as `-c envConfig=myEnvStageXxx` ') }
...
export function getProperty(properties: Record<string, any>, propName: string): string {
  if (!properties[propName] || properties[propName].trim().length === 0) { throw new Error(propName + ' does not exist or is empty') }

  return properties[propName]
}

export function decryptProperty(properties: Record<string, any>, propName: string): string {
